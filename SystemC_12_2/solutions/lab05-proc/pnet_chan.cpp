...
, addr        ( (sc_lv<2>) addr )
{
  ...
  SC_METHOD ( cpu_read_process  ) ; // read bytes from cpu data input port
    sensitive << p_cpu_read.data_written() ;
    dont_initialize() ;
  SC_METHOD ( cpu_write_process ) ; // write bytes to cpu data output port 
    sensitive << cpu_fifo.data_written_event() ;
    dont_initialize() ;
  SC_METHOD ( net_read_process  ) ; // read bytes from net data input port
    sensitive << p_net_read.data_written() ;
    dont_initialize() ;
  SC_THREAD ( net_write_process ) ; // write bytes to net data output port
}


// READ ADDR/DATA BYTE FROM EXTERNAL CPU FIFO THROUGH INPUT PORT
...
void pnet_chan::cpu_read_process ( )
{
  ...
  { // DATA BYTE
    ...
    { // UPLOAD TO CPU
      sc_assert ( cpu_fifo.nb_write ( frame ) ) ;
    }
    else
    { // FORWARD TO NET
      sc_assert ( net_fifo.nb_write ( frame ) ) ;
    }
  }
}
  ...
  while ( cpu_fifo.num_available() )
  {
    ...
    if ( VERBOSITY > 1 )
      cout << "time: "   << setw(5) << sc_simulation_time()
           ...
           << endl;
    sc_assert ( p_cpu_write.nb_write ( (sc_lv<6>(32), addr) ) ) ;
    if ( VERBOSITY > 1 )
      cout << "time: "   << setw(5) << sc_simulation_time()
           << " cycle: " << setw(5) << sc_delta_count()
           << " - " << name() << " send cpu data " << dec << data.to_uint()
           << endl;
    sc_assert ( p_cpu_write.nb_write ( data ) ) ;
  }
}
